...
void lista_reversa(Lista *li){
	Lista *a;
	//Procura o ulimo (elemento com prox=NULL)
	for(a=li;a->prox!=NULL;a=a->prox);
	//percoree no sentido inverso
	for(;a!=NULL;a=a->ant){
		printf("%d\t",a->info);
	}
	printf("\n");
...
void vizinhos(Lista *item){
	if(item->ant == NULL && item->prox != NULL){
		printf("\nVizinho anterior: Nunhum\nProximo vizinho: %d\n");
	}
	else{
		if(item->ant == NULL && item->prox == NULL){
			printf("Este item não tem vizinho");
		}
		else{
			printf("\nVizinho anterior: %d\nProximo vizinho: Nenhum\n");
		}
	}
...
void menu(){
     ...
     printf("8. Mostra reverso\n");//void lista_reversa(Lista *t)
	 
	 printf("10. Mostra vizinhos\n");
	 printf("9. Fim\n");
}     
main(){
 ...
 do{
	...
	case 8://mostra a lista reversamente
		lista_reversa(lst);
		break;
